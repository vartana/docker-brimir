...
CMD rake db:create && rake db:schema:load && rake assets:precompile && perl -p -i -e 's/config.serve_static_assets .*/config.serve_static_assets = true/' config/environments/production.rb && rails server -e production 
